...
my $wgtNewFile	     = $ConfigMap{"wgtNewFile"};
...
# making centrality directories, and lib links
 ...
 foreach $runcent(@RunCents){
        if(-e "$jobDir/$runcent.$weightFile") {
        print `cp $jobDir/$runcent.$weightFile $jobDir/$jobName/$cent \n`;
        }
 }
...
# producing shell script
 ...
 open (shellScript,">$jobDir/$jobName/$cent/$runJob");
  ...
  print shellScript "cp -Lr \$prototypeWDIR/cen*.$weightFile . \n";
  ...
  foreach $RunCent(@RunCents) {
	...
	print shellScript "root -b -q '$macroName++($centrality,1)' >& \${logfile} \n";
	print shellScript "if ( -e \"cen$centrality.$yieldFile\" || -e \"cen$centrality.$cumulFile\") then \n";
  	print shellScript "cp -f cen$centrality.$yieldFile  \$prototypeWDIR/sched\$JOBID.cen$centrality.$yieldFile \n";
  	...
  	print shellScript "cp -f cen$centrality.$wgtNewFile \$prototypeWDIR/sched\$JOBID.cen$centrality.$wgtNewFile \n";
