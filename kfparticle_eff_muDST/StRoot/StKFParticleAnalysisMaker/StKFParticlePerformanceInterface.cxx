...
void StKFParticlePerformanceInterface::WriteHistosCurFile( TObject *obj ){
  ...
  if( !obj->IsFolder() ) obj->Write();
    ...
    { 
      if (!obj1->IsFolder()) WriteHistosCurFile(obj1);
      else 
      {
        TDirectory *temp = sub->GetDirectory(obj1->GetName());
        string path = temp->GetPath();
        bool isNeeded = path.find("Lambda") != std::string::npos || path.length() <= 64; // if contains Lambda or is a top level folder
        isNeeded = isNeeded && path.find("PrimaryVertexQA") == std::string::npos; // if not PrimaryVertexQA
        if (isNeeded) WriteHistosCurFile(obj1);
      }
    }
    cur->cd();
    TFile::CurrentFile() = currentFile;
