              ...
              int A_PDG,int B_PDG,int Mode = 0) // Mode = 0: PDGMult 为vector长度
{
    ...
    std::vector<int>                      A_IfFilledInMix;
    ...
    std::vector<int>                      B_IfFilledInMix;
    // used as array
    ...
    for (int EntriesID = 0 ; EntriesID < nentries ; EntriesID++){
        ...
        if ((EntriesID+1)%200 == 0) {
            cout<<"Mix_A_Px.size() = "<<sizeof(Mix_A_Px[0][0][0][0][0])<<endl;
            cout<<"Mix_B_Px.size() = "<<sizeof(Mix_B_Px[0][0][0][0][0])<<endl;
        }
        // cout<<mult<<endl;
        // if(b>7){continue;}
        // cout<<"There OK"<<endl;
        ...
        A_IfFilledInMix.clear();B_IfFilledInMix.clear();
