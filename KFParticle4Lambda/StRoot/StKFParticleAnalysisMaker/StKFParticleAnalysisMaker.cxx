...
Int_t StKFParticleAnalysisMaker::Make() 
{
	...
	float DiffDVZCenter = -1.51381;// d+Au@200GeV RUN 16
	// if (!(6<fabs(vpdVz-VertexZ) && fabs(vpdVz-VertexZ)<30 && fabs(vpdVz)<20)) return kStOK; // band test
	if (!(fabs(vpdVz-VertexZ)<3 && 20<fabs(vpdVz-DiffDVZCenter) && fabs(vpdVz-DiffDVZCenter)<35)) return kStOK; // center test
